...
void NewCube::Render()
{
	if (_renderActive)
	{
		...
		// Texture
		_pDeviceContext->PSSetSamplers(0, 1, _samplerState.GetAddressOf());
...
HRESULT NewCube::SetTexture(const wchar_t* textureFilePath)
{
	...
	_pDeviceContext->PSSetShaderResources(0, 1, texSRV.GetAddressOf());
	...
	// 샘플러 상태 객체 생성
	hr = _pDevice->CreateSamplerState(&samplerDesc, &_samplerState);
	if (FAILED(hr))
	{
		return hr;
	}
